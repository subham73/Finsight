...
def check_op_forecast(
    ...
    """Check if OP ID with specific forecast type exists and validate project number for OB forecasts"""
    
    if not op_ids or not forecast_type:
        raise HTTPException(status_code=400, detail="op_ids and forecast_type are required")
    
    # Check if project with this OP ID exists
    ...
    if not existing_project:
        # New OP ID
        if forecast_type == "OB" and not project_number:
            raise HTTPException(status_code=400, detail="Project Number is required for new OP ID with OB forecast")
        
        # For new OP ID with OB, check if project_number already exists
        if forecast_type == "OB" and project_number:
            ...
            if existing_project_with_number:
                # Project number exists, will aggregate
                return {
                    "exists": False, 
                    "is_new_op": True,
                    "will_aggregate": True,
                    "aggregate_with_project_id": str(existing_project_with_number.id),
                    "message": f"Will aggregate OB forecasts with existing project number {project_number}"
                }
    ...
    # Existing OP ID
    if forecast_type == "OB":
        # Check if OB forecast already exists for this OP ID
        existing_ob_forecast = db.query(forecast_model.Forecast).filter(
            forecast_model.Forecast.project_id == existing_project.id,
            forecast_model.Forecast.forecast_type == "OB"
        ).first()
        
        if existing_ob_forecast:
            raise HTTPException(status_code=400, detail="OP ID already has an OB forecast")
    
    # For TB forecasts, multiple are allowed
    return {
        "exists": False,
        "is_new_op": False,
...
def get_projects_with_forecasts(
    ...
    for p in projects:
        ...
        for f in existing_forecasts:
            forecast_values[f.month] = float(f.amount)
            actual_values[f.month] = float(f.actuals)
...
def get_project_by_id(project_id: UUID, db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    for f in existing_forecasts:
        forecast_values[f.month] = float(f.amount)
        actuals_values[f.month] = float(f.actuals)
                ...
                if existing_project_with_number:
                    ...
                    for f in project.forecasts:
                        ...
                        if existing_forecast:
                            # Convert to Decimal before addition
                            existing_forecast.amount += Decimal(str(f.amount))
                            existing_forecast.forecast_usd += Decimal(str(forecast_usd))
                        else:
                            new_forecast = forecast_model.Forecast(
                                ...
                                year=f.year,
                                month=f.month,
                                amount=Decimal(str(f.amount)),  # Ensure Decimal
                                forecast_usd=Decimal(str(forecast_usd)),  # Ensure Decimal
...
def get_project(project_number: str, op_ids: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if project:
            ...
            #     forecast_model.Forecast == project.id
            # ).all()

            # print(forecast_values.id)

            return {
                "project": project,
                "manager_name": manager.name,
                # "forecast": [fore[0] for fore in forecast]
            }

    except Exception as Ex:
        print(Ex)
        return { "message": f"Unable to fetch project" }
    ...
    try:
        ...
        if source_countries:
            ...
            # return [ { "source_country": source } for source in source_countries]
             return [{"name": country[0], "label": country[0]} for country in source_countries]
...
def get_all_project_name(db: Session = Depends(get_db)):
    ...
    try:
        ...
        if projectNames:
            ...
            return [{"name": projectName[0], "label": projectName[0]} for projectName in projectNames]
...
def get_all_customer_name(db: Session = Depends(get_db)):
    ...
    try:
        ...
        if customerNames:
            ...
            return [{"name": customerName[0], "label": customerName[0]} for customerName in customerNames]
...
def get_all_customer_group(db: Session = Depends(get_db)):
    ...
    try:
        ...
        if customerGroups:
            ...
            return [{"name": customerGroup[0], "label": customerGroup[0]} for customerGroup in customerGroups]
...
def get_all_vertical(db: Session = Depends(get_db)):
    ...
    try:
        ...
        if verticals:
            ...
            return [{"name": vertical[0], "label": vertical[0]} for vertical in verticals]
...
def get_all_project_type(db: Session = Depends(get_db)):
    ...
    try:
        ...
        if projectTypes:
            ...
            return [{"name": projectType[0], "label": projectType[0]} for projectType in projectTypes]
...
def get_all_group(db: Session = Depends(get_db)):
    ...
    try:
        ...
        if projectGroups:
            ...
            return [{"name": projectGroup[0], "label": projectGroup[0]} for projectGroup in projectGroups]
...
def get_all_execution_country(db: Session = Depends(get_db)):
    ...
    try:
        ...
        if executionCountries:
            ...
            return [{"name": executionCountry[0], "label": executionCountry[0]} for executionCountry in executionCountries]
